...
- **Implementación**: Se utiliza `React.lazy()` y `<Suspense>` en [App.jsx](frontend/src/App.jsx) para cargar bajo demanda las vistas pesadas:
  - `Home`: Carga inmediata para asegurar un **First Contentful Paint (FCP)** ultra rápido.
  - `AdminPanel` y `SocioPanel`: Cargados perezosamente. El chunk pesado de administración (`AdminPanel` con librerías de gráficos como Recharts) solo se descarga cuando un administrador inicia sesión y accede al panel.
...
- **Implementación**: En [Home.jsx](frontend/src/views/Home.jsx), se crearon componentes de esqueleto (`CampaignSkeleton` y `NewsSkeleton`) que emulan la forma del contenido real con animaciones de pulso mientras se reciben los datos de las APIs, eliminando el parpadeo y la frustración visual.
...
Se auditaron y verificaron las mejores prácticas de SEO en [index.html](frontend/index.html):
...
- **Semántica y Accesibilidad**: Uso de etiquetas semánticas de HTML5 (`<main>`, `<header>`, `<footer>`, `<section>`) y adición de descripciones de accesibilidad `aria-label` en enlaces interactivos (ej: tarjetas de campañas y modal de compartir).
...
- **Implementación**: Se utiliza `express-rate-limit` para prevenir ataques de denegación de servicio (DDoS) o fuerza bruta en accesos, donaciones y operaciones sensibles.
- **Soporte de Proxy**: Se activó `app.set('trust proxy', 1)` en [index.js](backend/index.js) de Express para leer la cabecera `X-Forwarded-For` de los balanceadores de carga de Render o Vercel, evitando bloquear erróneamente a todos los usuarios bajo una sola IP de proxy.
...
- **Notificaciones por HTTPS (Resend API)**: Despacho de correos transaccionales por el puerto seguro 443 con la API REST de Resend, neutralizando bloqueos de puertos SMTP en entornos cloud serverless.

---

## 📋 5. Estado del Lanzamiento Oficial (Producción Activa)

> [!NOTE]
> El candado provisorio de contraseña implementado durante la etapa de pruebas del equipo fue retirado exitosamente de [main.jsx](frontend/src/main.jsx).
> 
> La plataforma se encuentra completamente productiva, abierta al público y conectada a la infraestructura en la nube (Vercel para Frontend, Render para Backend API, MongoDB Atlas para documentos y PostgreSQL para la persistencia relacional ACID).
